...
public class DisplayLabel : MonoBehaviour
{
    ...
    void Update()
    {
		...
		if (hasHit)
		{
			...
			if (OVRInput.GetDown(OVRInput.Button.SecondaryIndexTrigger) && fieldFlag == 0)
			{
				battleField = Instantiate(battleFieldPrefab, hitPoint, Quaternion.identity);
				fieldFlag = 1;
			}
			if (OVRInput.GetDown(OVRInput.Button.SecondaryHandTrigger) && fieldFlag == 1)
			{
				Destroy(battleField);
				fieldFlag = 0;
			}
